...
export function Cards({ notas, eliminarNota }) {
    return (
            <div className="p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-5">
                {
                    notas.map((item) => (
                        <div className="rounded overflow-hidden shadow-lg bg-yellow-200" key={item.id}>
                            ...
                            <div className="px-6 pt-4 pb-2">
                                <button type="button" className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 ">Ver archivos</button>
